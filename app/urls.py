from django.urls import path
from django.contrib.auth import views as auth_views
from . import views

urlpatterns = [
    path('', views.dashboard, name='dashboard'),
    path('busca/', views.busca_global, name='busca_global'),
    path('gabriel/', views.area_gabriel, name='area_gabriel'),
    path('gabriel/tarefa/<int:tarefa_id>/concluir/', views.concluir_tarefa_pessoal, name='concluir_tarefa_pessoal'),
    path('gabriel/tarefa/<int:tarefa_id>/reabrir/', views.reabrir_tarefa_pessoal, name='reabrir_tarefa_pessoal'),
    path('gabriel/tarefa/<int:tarefa_id>/prioridade/', views.alterar_prioridade_tarefa_pessoal, name='alterar_prioridade_tarefa_pessoal'),
    path('gabriel/tarefa/<int:tarefa_id>/excluir/', views.excluir_tarefa_pessoal, name='excluir_tarefa_pessoal'),
    path('gabriel/tarefa/<int:tarefa_id>/comentario/', views.adicionar_comentario_tarefa_pessoal, name='adicionar_comentario_tarefa_pessoal'),
    path('gabriel/comentario/<int:comentario_id>/editar/', views.editar_comentario_tarefa_pessoal, name='editar_comentario_tarefa_pessoal'),
    path('gabriel/comentario/<int:comentario_id>/excluir/', views.excluir_comentario_tarefa_pessoal, name='excluir_comentario_tarefa_pessoal'),
    path('login/', auth_views.LoginView.as_view(template_name='app/login.html'), name='login'),
    path('logout/', views.sair, name='logout'),
    path('tarefas/', views.tarefas, name='tarefas'),
    path('comercial/', views.comercial, name='comercial'),
    path('tarefa/criar/', views.criar_tarefa, name='criar_tarefa'),
    path('tarefa/<int:tarefa_id>/atribuir/', views.atribuir_tarefa, name='atribuir_tarefa'),
    path('tarefa/<int:tarefa_id>/mover/', views.mover_tarefa, name='mover_tarefa'),
    path('tarefa/<int:tarefa_id>/gerar-implantacao/', views.gerar_ticket_implantacao, name='gerar_ticket_implantacao'),
    path('tarefa/<int:tarefa_id>/', views.detalhes_tarefa, name='detalhes_tarefa'),
    path('tarefa/<int:tarefa_id>/comentario/<int:comentario_id>/editar/', views.editar_comentario_ticket, name='editar_comentario_ticket'),
    path('tarefa/<int:tarefa_id>/comentario/<int:comentario_id>/excluir/', views.excluir_comentario_ticket, name='excluir_comentario_ticket'),
    path('anexo/<int:anexo_id>/baixar/', views.baixar_anexo_ticket, name='baixar_anexo_ticket'),
    path('anexo/<int:anexo_id>/remover/', views.remover_anexo_ticket, name='remover_anexo_ticket'),
    path('tarefa/<int:tarefa_id>/encerrar/', views.encerrar_tarefa, name='encerrar_tarefa'),
    path('tarefa/<int:tarefa_id>/reabrir/', views.reabrir_tarefa, name='reabrir_tarefa'),
    path('notificacoes/<int:notificacao_id>/lida/', views.marcar_notificacao_lida, name='marcar_notificacao_lida'),
    path('notificacoes/marcar-todas-lidas/', views.marcar_todas_notificacoes_lidas, name='marcar_todas_notificacoes_lidas'),
    path('notificacoes/status/', views.status_notificacoes, name='status_notificacoes'),
    path('financeiro/despesa/<int:despesa_id>/confirmar-popup/', views.confirmar_despesa_popup, name='confirmar_despesa_popup'),
    path('encerradas/', views.tarefas_encerradas, name='tarefas_encerradas'),
    path('calendario/', views.calendario, name='calendario'),
    path('financeiro/', views.financeiro, name='financeiro'),
    path('rotina/', views.rotina, name='rotina'),
    path('rotina/<int:rotina_id>/concluir/', views.concluir_rotina, name='concluir_rotina'),
    path('clientes/', views.clientes, name='clientes'),
    path('cliente/criar/', views.criar_cliente, name='criar_cliente'),
    path('cliente/<int:cliente_id>/', views.detalhes_cliente, name='detalhes_cliente'),
    path('cliente/<int:cliente_id>/inativar/', views.inativar_cliente, name='inativar_cliente'),
    path('cliente/<int:cliente_id>/reativar/', views.reativar_cliente, name='reativar_cliente'),
    path('clientes-inativos/', views.clientes_inativos, name='clientes_inativos'),
    path('erros-conhecidos/', views.erros_conhecidos, name='erros_conhecidos'),
    path('erro-conhecido/criar/', views.criar_erro_conhecido, name='criar_erro_conhecido'),
    path('erro-conhecido/<int:erro_id>/editar/', views.editar_erro_conhecido, name='editar_erro_conhecido'),
    path('erro-conhecido/<int:erro_id>/', views.detalhes_erro_conhecido, name='detalhes_erro_conhecido'),
    path('erro-conhecido/anexo/<int:anexo_id>/baixar/', views.baixar_anexo_erro_conhecido, name='baixar_anexo_erro_conhecido'),
    path('erro-conhecido/anexo/<int:anexo_id>/remover/', views.remover_anexo_erro_conhecido, name='remover_anexo_erro_conhecido'),
    path('sugestoes-desenvolvimento/', views.sugestoes_desenvolvimento, name='sugestoes_desenvolvimento'),
    path('sugestao-desenvolvimento/criar/', views.criar_sugestao_desenvolvimento, name='criar_sugestao_desenvolvimento'),
    path('sugestao-desenvolvimento/<int:sugestao_id>/editar/', views.editar_sugestao_desenvolvimento, name='editar_sugestao_desenvolvimento'),
    path('sugestao-desenvolvimento/<int:sugestao_id>/', views.detalhes_sugestao_desenvolvimento, name='detalhes_sugestao_desenvolvimento'),
    path('sugestao-desenvolvimento/anexo/<int:anexo_id>/baixar/', views.baixar_anexo_sugestao_desenvolvimento, name='baixar_anexo_sugestao_desenvolvimento'),
    path('sugestao-desenvolvimento/anexo/<int:anexo_id>/remover/', views.remover_anexo_sugestao_desenvolvimento, name='remover_anexo_sugestao_desenvolvimento'),
    path('procedimentos/', views.procedimentos_internos, name='procedimentos_internos'),
    path('procedimento/criar/', views.criar_procedimento, name='criar_procedimento'),
    path('procedimento/<int:procedimento_id>/editar/', views.editar_procedimento, name='editar_procedimento'),
    path('procedimento/<int:procedimento_id>/', views.detalhes_procedimento, name='detalhes_procedimento'),
    path('procedimento/anexo/<int:anexo_id>/baixar/', views.baixar_anexo_procedimento, name='baixar_anexo_procedimento'),
    path('procedimento/anexo/<int:anexo_id>/remover/', views.remover_anexo_procedimento, name='remover_anexo_procedimento'),
    path('releases/', views.releases, name='releases'),
    path('caixa-de-entrada/', views.caixa_entrada, name='caixa_entrada'),
    path('caixa-de-entrada/<int:comunicacao_id>/leitura/', views.alterar_leitura_comunicacao, name='alterar_leitura_comunicacao'),
    path('releases/solicitacoes/', views.solicitacoes_releases, name='solicitacoes_releases'),
    path('releases/solicitacao/<int:solicitacao_id>/comentar/', views.comentar_solicitacao, name='comentar_solicitacao'),
    path('releases/solicitacao/<int:solicitacao_id>/status/', views.alterar_status_solicitacao, name='alterar_status_solicitacao'),
]
